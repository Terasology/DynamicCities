...
public class SiteFacetProvider implements ConfigurableFacetProvider {
    ...
    public void process(GeneratingRegion region) {
        ...
        if (roughnessFacet.getMeanDeviation() < 0.3f && roughnessFacet.getMeanDeviation() > 0
            ...
            int population = Math.round(sizeNoiseGen.noise(minPos.getX(), minPos.getY())*config.maxPopulation);
    ...
    private static class SiteConfiguration implements Component {
        ...
        @Range(label = "Maximum town population", description = "Maximum town size in blocks", min = 10, max = 350, increment = 10, precision = 1)
        private int maxPopulation = 256;
