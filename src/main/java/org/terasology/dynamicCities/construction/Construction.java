...
public class Construction extends BaseComponentSystem {
        ...
        for (Vector2ic pos : area) {
            ...
            if (y <= meanHeight) {
                for (int i = y; i <= meanHeight; i++) {
                    ...
                    worldProvider.getWorldEntity().send(new SetBlockEvent(JomlUtil.from(setPos), filler));
                }
            }
            if (y >= meanHeight) {
                for (int i = y; i > meanHeight; i--) {
                    setPos.set(pos.x(), i, pos.y());
                    worldProvider.getWorldEntity().send(new SetBlockEvent(JomlUtil.from(setPos), air));
                }
            }
        }
    ...
    public boolean buildParcel(DynParcel dynParcel, EntityRef settlement) {
        ...
        if (building.isScaledDown) {
            Vector2i difference = dynParcel.shape.getSize(new Vector2i()).sub(building.minSize);
            dynParcel.shape = new BlockArea(dynParcel.shape.getMin(new Vector2i()).add(difference.x()/2, difference.y()/2), dynParcel.shape.getMax(new Vector2i()).sub(difference));
    ...
    public RoadStatus buildRoadParcel(RoadParcel parcel, EntityRef settlement) {
        ...
        final Vector2ic rectExpansionFactor = new Vector2i(expWidth, expHeight);
        ...
        for (int i = 0; i < parcel.rects.size(); i++) {
            ...
            segment.height = flatten(segment.rect.expand(rectExpansionFactor, new BlockArea(BlockArea.INVALID)), segmentHeight);
