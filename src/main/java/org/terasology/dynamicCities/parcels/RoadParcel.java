...
public class RoadParcel implements Parcel {
    ...
    public Set<BlockAreac> expand(int dx, int dy) {
        Set<BlockAreac> expandedRects = new HashSet<>();
        if (!rects.isEmpty()) {
            for (RoadSegment roadRect : rects) {
                expandedRects.add(roadRect.rect.expand(dx, dy, new BlockArea(BlockArea.INVALID)));
            }
        }
    ...
    public boolean isNotIntersecting(BlockAreac rect) {
        for (RoadSegment segment : rects) {
            if (segment.rect.intersectsBlockArea(rect)) {
                return false;
            }
        }
        return true;
